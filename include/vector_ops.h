...
namespace backtester
{
    ...
    public:
        template<typename T>
        static std::vector<T> Take(const std::vector<T> &v, const int n) {
            return std::vector<T>(v.begin(), v.begin() + n);
        }

        template<typename T>
        static std::vector<T> Take(const std::vector<T> &v, int first, int last) {
            if (first < 0)
                first = v.size() - first;
            if (last < 0)
                last = v.size() - last;
        ...
        template<typename T>
        static std::vector<T> Drop(const std::vector<T> &v, int n) {
            if (n > 0)
        ...
        template<typename T>
        static bool VectorContainsQ(const std::vector<T> &v, const T &key) {
        ...
        template<typename T>
        static T Total(const std::vector<T> &v) {
        ...
        /// \tparam T Numeric type.
        template<typename T>
        static T Multiply(const std::vector<T> &v) {
        ...
        /// \param b The scalar.
        template<typename T>
        static std::vector<T> Subtract(std::vector<T> a, const T b) {
            for (T &el: a)
                el -= b;
            return a;
        }
        ...
        template<typename T>
        static std::vector<int> Sign(const std::vector<T> &v) {
        ...
        template<typename T>
        static std::vector<int> CountSubsequences(const std::vector<T> &v) {
